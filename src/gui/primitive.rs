...
impl PrimitiveRenderer {
    ...
    pub fn new(ctx: &WgpuContext, format: wgpu::TextureFormat) -> Self {
        ...
        let screen_buffer = ctx.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
    ...
    pub fn draw_circle(&mut self, x: f32, y: f32, radius: f32, color: [f32; 4]) {
        self.push_quad(x, y, radius * 2.0, radius * 2.0, [0.0, 0.0], [1.0, 1.0], color, 1);
    }
    ...
    pub fn prepare(&mut self, ctx: &WgpuContext, width: u32, height: u32) {
        ...
        if self.screen_size != [width as f32, height as f32] {
            ...
            ctx.queue.write_buffer(&self.screen_buffer, 0, bytemuck::cast_slice(&data));
        }
    ...
    pub fn render<'a>(&'a self, pass: &mut wgpu::RenderPass<'a>) {
        ...
        pass.set_vertex_buffer(0, self.vertex_buffer.slice(0..((self.vertices.len() * std::mem::size_of::<Vertex>()) as u64)));
